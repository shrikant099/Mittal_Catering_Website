...
export const metadata: Metadata = {
  ...
  description:
    "Your order has been placed successfully with Mittal Catering. Track your order details and delivery status here.",
    ...
    return (
      <main className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="max-w-lg w-full bg-[#121212] p-6 sm:p-8 rounded-2xl text-center shadow-xl">
          ...
          <Link
            href="/menu"
            className="inline-block mt-6 bg-primary text-black px-6 py-3 rounded-xl font-bold"
          >
            Back to Menu
          </Link>
        </div>
      ...
      <div className="max-w-4xl mx-auto bg-[#121212] rounded-2xl shadow-2xl p-4 sm:p-8">
        ...
        <div className="text-center mb-6 sm:mb-10">
          ...
          <p className="text-white/70 mt-2">
            Your meal will be delivered to your seat.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <section className="bg-[#1A1A1A] rounded-xl p-4 sm:p-5">
            ...
            <ul className="text-white/80 space-y-1 text-sm sm:text-base">
              ...
              <li>
                <b>Mobile:</b> {order.customer.mobileNumber}
              </li>
              <li>
                <b>Train:</b> {order.customer.trainNumber}
              </li>
              <li>
                <b>PNR:</b> {order.customer.pnr}
              </li>
              <li>
                <b>Coach:</b> {order.customer.coach}
              </li>
              <li>
                <b>Seat:</b> {order.customer.seat}
              </li>
              {order.customer.instructions && (
                <li>
                  <b>Instructions:</b> {order.customer.instructions}
                </li>
              )}
            </ul>
          </section>
          ...
          <section className="bg-[#1A1A1A] rounded-xl p-4 sm:p-5">
            ...
            <ul className="space-y-2">
              {order.items.map((i: any) => (
                <li
                  key={i._id}
                  className="flex justify-between text-white/90 text-sm sm:text-base"
                >
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
          <Link
            href="/menu"
            className="bg-primary text-black px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold text-center"
          >
            Order Again
          </Link>
          <Link
            href="/"
            className="border border-primary text-primary px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold text-center"
          >
            Back to Home
          </Link>
        </div>
